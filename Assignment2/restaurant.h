...
public:
	...
	void print() {
		for (int i = 0; i < MAXSIZE / 2; i++) {
			cout << i << ": " << v[i].name << " - " << v[i].result << " - " << v[i].tableId << " - " << v[i].areaId << endl;
		}
	}
	void printHT() {
		for (int i = 0; i < MAXSIZE / 2; i++) {
			if (v[i].result != -1) {
				cout << v[i].tableId << "-" << v[i].result << "-" << v[i].order << endl;
			}
		}
	}
};
...
public:
	...
	void printAVL() {
		...
		while (!q.empty()) {
			NodeAVL* x = q.front();
			q.pop();
			cout << x->key.tableId << "-" << x->key.result << "-" << x->key.order << endl;
...
public:
	...
	void print() {
		for (unsigned int i = 0; i < data.size(); i++) {
			cout << data[i].name << " " << data[i].order;
		}
		cout << endl;
	}
	void printMH(int index) {
		if (index >= data.size()) return;
		cout << data[index].tableId << "-" << data[index].order << endl;
		printMH(2 * index + 1);
		printMH(2 * index + 2);
	}
};
...
void HuffmanCodes(map <char, int>& m, map<char, string>& result) {
	...
	//	cout << minHeap_copy.top()->frequency << "-" << minHeap_copy.top()->data << " ";
	//	minHeap_copy.pop();
	//}
	//cout << endl;
